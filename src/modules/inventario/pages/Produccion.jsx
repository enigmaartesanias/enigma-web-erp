...
import { FaEdit, FaTrash, FaArrowLeft, FaSave, FaTimes, FaBox, FaMoneyBillWave, FaHammer } from 'react-icons/fa';
                    ...
                    {formData.tipo_produccion === 'PEDIDO' && !editingId && (
                        <div className="bg-amber-50 p-4 rounded-xl border border-amber-200">
                            ...
                            <select
                                ...
                                className="w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500 border p-2"
                                required
                            >
                                <option value="">-- Selecciona un producto --</option>
                                {pedidosFiltradosDropdown.map(prod => (
                                    <option key={`${prod.id_pedido}-${prod.id_detalle}`} value={`${prod.id_pedido}-${prod.id_detalle}`}>
                                        #{prod.id_pedido} - {prod.nombre_cliente} -- {prod.nombre_producto} (Cant: {prod.cantidad}) -- S/ {parseFloat(prod.precio_unitario || 0).toFixed(2)}
                                    </option>
                                ))}
                            </select>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    ...
                                    disabled={formData.tipo_produccion === 'PEDIDO' && formData.pedido_id}
                                    required
                                >
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    name="cantidad"
                                    min="1"
                                    className={`w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 border p-2 ${formData.tipo_produccion === 'PEDIDO' && formData.pedido_id ? 'bg-gray-100 cursor-not-allowed' : ''}`}
                                    value={formData.cantidad}
                                    onChange={handleChange}
                                    readOnly={formData.tipo_produccion === 'PEDIDO' && formData.pedido_id}
                                    required
                                />
                            </div>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredProduccion.map((item) => (
                                <tr key={item.id_produccion} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-center text-sm font-semibold text-gray-900">{item.cantidad}</td>
                                    <td className="px-4 py-3 text-right text-sm text-gray-900">S/ {parseFloat(item.costo_total_unitario || 0).toFixed(2)}</td>
