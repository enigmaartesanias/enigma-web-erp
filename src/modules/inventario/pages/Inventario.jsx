...
import { FaPlus, FaEdit, FaTrash, FaSearch, FaArrowLeft, FaBox, FaHammer, FaChevronDown, FaChevronUp } from 'react-icons/fa';
        ...
        <div className="min-h-screen bg-gray-50 pb-10">
            ...
            <div className="bg-white shadow-sm border-b sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
                    ...
                    <button onClick={() => navigate('/inventario/nuevo')} className="bg-slate-800 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-slate-700">
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
                ...
                {!loading && pendientes.length > 0 && (
                    <div className="mb-6 bg-amber-50 border border-amber-100 rounded-xl overflow-hidden">
                        <button onClick={() => setPendientesExpanded(!pendientesExpanded)} className="w-full flex items-center justify-between px-4 py-3 text-amber-800 text-xs font-bold uppercase">
                            Producción en Taller ({pendientes.length}) {pendientesExpanded ? <FaChevronUp /> : <FaChevronDown />}
                        </button>
                        {pendientesExpanded && (
                            <div className="divide-y divide-amber-100 bg-white">
                                {pendientes.map(item => (
                                    <div key={item.id_produccion} className="flex items-center justify-between px-4 py-3">
                                        ...
                                        <button onClick={() => handleIngresar(item)} className="px-3 py-1 bg-amber-600 text-white text-[10px] rounded-lg">INGRESAR</button>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <input type="text" placeholder="Buscar por código o nombre..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="p-3 border rounded-lg text-sm" />
                    <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="p-3 border rounded-lg text-sm">
                        ...
                        <tbody className="divide-y">
                            {filteredProductos.map(p => (
                                <tr key={p.id} className="hover:bg-gray-50">
                                    <td className="p-4 font-mono font-bold text-gray-700">{p.codigo_usuario}</td>
                                    <td className="p-4">
                                        <div className="font-semibold">{p.nombre}</div>
                                        <div className="text-gray-400 italic">{p.material}</div>
                                    </td>
                                    <td className="p-4 text-center font-bold text-green-600">{p.stock_actual}</td>
                                    <td className="p-4 text-right font-bold">S/ {Number(p.precio).toFixed(2)}</td>
            ...
            {showStockIngressModal && (
                <StockIngressModal item={selectedPendiente} onSuccess={handleIngresarSuccess} onCancel={() => setShowStockIngressModal(false)} />
            )}
